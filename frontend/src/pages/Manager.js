...
function Manage_Clubs(props) {
  ...
  return (
    ...
    <h1 className="card text-white bg-primary mb-1" styleName="max-width: 25rem;">Manage User Profile</h1>
    <div className="card-body">
    <h5 className="card text-white bg-dark mb-3">Join a Club</h5>
    <form onSubmit={addClubsHandler}>
      <span className="card-text">
        ...
        <input type="text" className="mb-2 form-control desIn" onChange={event => setDesc(event.target.value)} placeholder='Role'/>
        <input type="number" className="mb-2 form-control sizeIn" onChange={event => setSize(event.target.value)} placeholder='Priority'/>
        <label> Favorite this club: <input type="checkbox" onChange={event => setStatus(event.target.checked)}/></label>
        <input type="text" className="mb-2 form-control emailIn" onChange={event => setEmail(event.target.value)} placeholder='Notes'/>
        <button className="btn btn-outline-primary mx-2 mb-3" style={{'borderRadius':'50px', "font-weight":"bold"}}>Join</button>
      </span>
    </form>

    <h5 className="card text-white bg-dark mb-3">Joined Clubs:</h5>
      <span>
      ...
      <h5 className="card text-white bg-dark mb-3">Favorited Clubs:</h5>
      <span>
        <select value={sortMethod} onChange={(event) => setSortMethod(event.target.value)}>
          <option value="nameAsc">Sort by name asc</option>
          <option value="nameDesc">Sort by name desc</option>
      ...
      <h5 className="card text-white bg-dark mb-3">Favorited Events:</h5>
      <span>
        <select value={sortMethod} onChange={(event) => setSortMethod(event.target.value)}>
          <option value="nameAsc">Sort by name asc</option>
          <option value="nameDesc">Sort by name desc</option>
